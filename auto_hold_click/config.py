...
class Config:
    ...
    JSON設定ファイル(デフォルトは 'config.json')からトグルキーと
    マウスボタンの設定を読み込む。
    ...
    def __init__(self, config_file: str, logger_helper: LoggerHelper = None) -> None:
        ...
        Args:
            config_file (str): 設定ファイルのパス。
            logger_helper (LoggerHelper, optional): ログ出力用のLoggerHelper。
                デフォルトはNone。
        """
    ...
    def load(self) -> None:
        ...
        Raises:
            AutoHoldClickError: 設定ファイルの読み込みに失敗した場合。
        """
        try:
            with Path.open(self.config_file, encoding=self.CONFIG_FILE_ENCODING) as f:
    ...
    def _get_config_value(self, key: str) -> str:
        ...
        if value is None:
            raise AutoHoldClickError(Messages.CONFIG_KEY_NOT_FOUND.format(key=key))
        if value == "":
            raise AutoHoldClickError(Messages.CONFIG_EMPTY_VALUE.format(key=key))
        return value
